...
def plot_winner_stability(data: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_xlim(0, 1)
    ax.set_xlabel("Conditional fresh-run win estimate · log-normal model")
    ax.set_title("Ranking stability depends on reported variation", loc="left", weight="bold", color=INK)
